...
$dbUsername = "u876421973_luong";
$dbPassword = "190594luong";
$dbName = "u876421973_chat";
...
switch($action) 
{
    ...
    . "left join users u on u.Id = m.fromuid WHERE `touid` = ".$userId." AND `read` = 0 LIMIT 0, 30 ";
			...
			$sqlstatus = "SELECT status.text, users.username, status.timeUp
						  FROM status
						  JOIN users
						  ON status.userID=users.ID";
			...
			if ($result = $db->query($sql))			
			{
					...
					if ($resultstatus = $db->query($sqlstatus))
					{
						while ($rowstatus = $db->fetchObject($resultstatus))
						{
							$out .= "<status  username='".$rowstatus->username."' text='".$rowstatus->text."' timeUp='".$rowstatus->timeUp."' />";
						}
					}
	...
	if ($userId = authenticateUser($db, $username, $password)) 
		...
		if (isset($_REQUEST['status']))
		{
			...
			$sql22 = "INSERT INTO `status` (`userID`, `text`, `timeUp`) VALUES ('".$userId."', '".$status."', '".$time."');";						
